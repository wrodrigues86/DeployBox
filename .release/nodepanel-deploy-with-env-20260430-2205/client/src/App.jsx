...
function CodeTab({ project, authHeaders, onSaved, t }) {
  ...
  async function openProjectFile(path) {
    if (!path || path === selectedFilePath) return
    try {
  ...
  useEffect(() => {
    ...
    $tree
      .on('select_node.jstree', (_evt, payload) => {
          ...
          <div className="space-y-2">
            <div className="flex max-w-full flex-wrap items-center gap-2">
              <div className="flex shrink-0 items-center gap-1.5 rounded-lg border border-panel-border bg-slate-950/50 p-1.5">
                <button
                  className="btn border-panel-accent bg-panel-accent/15 px-3 py-1.5 text-panel-accent hover:bg-panel-accent/20"
                  onClick={async () => {
      ...
      <div className={`min-h-0 flex-1 ${filesPaneHidden
        ...
        <div className="relative h-full min-h-0 flex-1 overflow-hidden rounded-lg border border-panel-border">
          ...
          <Editor
            ...
            language={fileLang}
            value={code}
            onChange={(value) => { setCode(value || ''); setFileDirty(true) }}
            theme="vs-dark"
            options={{
  ...
  return (
    <div>
      ...
      <div className="mb-2 flex gap-2">
        <button className="btn border-panel-accent text-panel-accent" onClick={async () => {
          try {
            const { data } = await api.post(`/projects/${project.id}/sql/run`, { sql }, { headers: authHeaders })
            setResult(data)
          } catch (e) {
