...
function listProjectTextFiles(project) {
  ...
  while (stack.length) {
    ...
    for (const entry of entries) {
      ...
      files.push({ path: nextRel.replace(/\\/g, '/'), size, updatedAt });
    }
  }
...
1. Edite o \`index.js\`.
2. Ajuste variÃ¡veis no \`.env\`.
3. Suba este ZIP na aba CÃ³digo > Subir ZIP.
...
function runGitClone({ repoUrl, branch, targetDir }) {
...
app.get('/api/settings', (req, res) => {
  return res.json({
    system_version: getAppSetting('system_version', '1.0.0'),
  });
});
...
app.put('/api/settings', (req, res) => {
  ...
  setAppSetting('system_version', nextVersion);
  return res.json({ ok: true, system_version: nextVersion });
});
...
app.get('/api/templates/project-zip', (req, res) => {
  ...
  const type = requestedType === 'worker' || requestedType === 'app' ? requestedType : 'api';
...
app.post('/api/projects/:id/clone-git', async (req, res) => {
  ...
  try {
    await runGitClone({ repoUrl, branch, targetDir: tempRepoDir });
    ...
    addLog(project.id, 'error', `Falha clone Git: ${error.message}`);
    return res.status(400).json({ error: error.message || 'git_clone_failed' });
  } finally {
...
app.post('/api/projects', async (req, res) => {
  ...
  const type = data.type === 'worker' ? 'worker' : data.type === 'app' ? 'app' : 'api';
  ...
  res.status(201).json(sanitizeProject(getProjectById(project.id)));
});
...
app.put('/api/projects/:id', async (req, res) => {
  ...
  if (!current) return res.status(404).json({ error: 'Projeto nÃ£o encontrado' });
...
app.post('/api/projects/stop-all', (req, res) => {
  ...
  for (const row of rows) stopContinuousWorker(row.id);
  return res.json({ ok: true, total: rows.length });
});

app.post('/api/system/restore-initial', (req, res) => {
  if (!assertFullAdmin(req, res)) return;
...
app.post('/api/projects/:id/sql/run', (req, res) => {
  ...
  if (!sql) return res.status(400).json({ error: 'SQL obrigatÃ³rio' });
...
app.get('/api/projects/:id/versions/:versionId', (req, res) => {
  ...
  if (!row) return res.status(404).json({ error: 'VersÃ£o nÃ£o encontrada' });
  res.json(row);
});
...
app.post('/api/projects/:id/restore/:versionId', async (req, res) => {
  ...
  if (!version) return res.status(404).json({ error: 'VersÃ£o nÃ£o encontrada' });

  writeProjectCode(project, version.code);
  await bootProjectRuntime(getProjectById(project.id));
  addLog(project.id, 'warning', `VersÃ£o restaurada: ${req.params.versionId}`);
...
app.post('/api/projects/:id/env', (req, res) => {
  ...
  if (!envKey) return res.status(400).json({ error: 'envKey obrigatÃ³rio' });
  ...
  persistEnvFile(project);
  addLog(project.id, 'info', `VariÃ¡vel atualizada: ${envKey}`);
  res.json({ ok: true });
});
...
app.post('/api/projects/:id/dependencies', async (req, res) => {
  ...
  if (!pkg) return res.status(400).json({ error: 'packageName obrigatÃ³rio' });
...
app.get('/api/projects/:id/files', (req, res) => {
  ...
  if (!assertProjectAccess(req, res, project.id)) return;
  ensureProjectFiles(project);
  return res.json({ files: listProjectTextFiles(project) });
});
...
app.post('/api/projects/:id/code', async (req, res) => {
  ...
  if (!code.trim()) return res.status(400).json({ error: 'code obrigatÃ³rio' });
  if (!assertWithinStorageLimit(req, res, Buffer.byteLength(code, 'utf8'))) return;

  const version = writeProjectCode(project, code);
  await bootProjectRuntime(getProjectById(project.id));
  addLog(project.id, 'info', `CÃ³digo salvo: ${version.versionTag}`);
...
app.post('/api/projects/:id/upload-zip', upload.single('file'), async (req, res) => {
  ...
  if (!project) return res.status(404).json({ error: 'Projeto nÃ£o encontrado' });
  if (!assertProjectAccess(req, res, project.id)) return;
  if (!req.file) return res.status(400).json({ error: 'Arquivo ZIP obrigatÃ³rio' });
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (!normalized || normalized.startsWith('..') || path.isAbsolute(normalized)) {
        return res.status(400).json({ error: `Caminho invÃ¡lido no ZIP: ${entry.entryName}` });
      }
...
app.get('/api/cron/preview', (req, res) => {
  ...
  if (!expression) return res.status(400).json({ error: 'Preset invÃ¡lido' });
...
app.get('/api/dashboard/stats', async (req, res) => {
  ...
  const projectsRows = isFullAdmin(req)
    ? db.prepare('SELECT id, name, slug, status, active FROM projects ORDER BY id DESC').all()
    : db
        .prepare(
          `SELECT p.id, p.name, p.slug, p.status, p.active
           FROM projects p
           INNER JOIN user_projects up ON up.project_id = p.id
           WHERE up.user_id = ?
           ORDER BY p.id DESC`,
...
bootAllProjects().then(() => {
  server.listen(port, () => {
    ...
    console.log('Login padrÃ£o: admin@nodepanel.local / admin123');
  });
});
